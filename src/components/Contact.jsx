...
const CONTACT_META = [
  ...
  { icon: MapPin, labelKey: 'location', valueKey: 'locationValue', href: null, color: 'text-pink-400', bg: 'bg-pink-400/[0.08]', border: 'border-pink-400/15' },
]
    ...
    <section id="contato" className="relative py-24 px-6">
      <div className="max-w-4xl mx-auto relative z-10" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-16">
          <p className="text-cyan-400 font-semibold text-sm tracking-widest uppercase mb-2">{ct.tagline}</p>
          <h2 className="section-title gradient-text">{ct.title}</h2>
          <p className="section-subtitle mx-auto mt-3">{ct.subtitle}</p>
        </motion.div>

        <div className="grid sm:grid-cols-2 gap-4 mb-10">
          {CONTACT_META.map((c, idx) => {
            const Tag = c.href ? 'a' : 'div'
            const props = c.href ? { href: c.href, target: '_blank', rel: 'noopener noreferrer' } : {}
            const displayLabel = c.labelKey ? ct[c.labelKey] : c.label
            const displayValue = c.valueKey ? ct[c.valueKey] : c.value
            return (
              <motion.div key={displayLabel} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 + idx * 0.1 }}>
                <Tag {...props} className={`flex items-center gap-4 p-5 rounded-xl border transition-all duration-300 ${c.bg} ${c.border} ${c.href ? 'cursor-pointer hover:scale-[1.02]' : ''}`} style={{ textDecoration: 'none' }}>
                  ...
                  <div>
                    <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-0.5">{displayLabel}</p>
                    <p className="text-sm text-slate-300 font-semibold">{displayValue}</p>
                  </div>
                </Tag>
              </motion.div>
            )
